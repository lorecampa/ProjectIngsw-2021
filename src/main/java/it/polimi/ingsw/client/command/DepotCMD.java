...
public class DepotCMD implements Command{
    ...
    public void doCommand() {
        ...
        if(CommandsUtility.isNotAResource(split[0])){
            PrintAssistant.instance.invalidParamCommand(cmd);
            return;
        }
        int value=CommandsUtility.stringToInt(split[1]);
        if(value==-1){
            PrintAssistant.instance.invalidParamCommand(cmd);
            return;
        }
        ResourceData resource= CommandsUtility.stringToResource(split[0], value);
    ...
    public void help() {
        ...
        rowHelp.add("The command will automatically understand based on the state of the game if you are have to subtract or add to depots!");
